...
for node, adjacencies in enumerate(G.adjacency()):
    ...
    node_trace['text'] += tuple(['AKT1' ])#tuple([node_info])
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Dropdown(
            value='Enter a value...',
            options = [{'label': i, 'value': i} for i in pkinfam['kinase_name'].unique().tolist()],
            id='input'
        ),
...
# Table
@app.callback(
    Output('click-data3', 'rows'),
    # [Input('basic-interactions2', 'clickData')])
    [Input('input', 'value')])
def display_click_data_3(value):

    # filter = clickData['points'][0]['text']

    df1 = results.loc[results['KinaseLabel'] == value].copy()
...
@app.callback(
    Output('click-data4', 'rows'),
    # [Input('basic-interactions2', 'clickData')])
    [Input('input', 'value')])
def display_click_data_4(value):

    # filter = clickData['points'][0]['text']
    df2 = results.loc[results['SubstrateLabel'] == value].copy()

    df2['Score'] = df2['Score'].apply(lambda x: np.round(x, 3))
    grouped2 = df2.groupby(['KinaseLabel', 'SubstrateLabel']).agg({'Score': ['min', 'max']})
    grouped2.columns = grouped2.columns.map('_'.join)
    grouped2 = grouped2.sort_values(by='Score_min', ascending=False).reset_index()
    grouped2['Score Range'] = grouped2.apply(lambda row: str(row['Score_min']) + ' - ' + str(row['Score_max']), axis=1)
    grouped2.drop(['Score_min', 'Score_max'], axis=1, inplace=True)
    grouped2 = grouped2.reset_index()
    print(grouped2.head())
    # df_aux = df_aux.sort_values(by='PMID_Count', ascending=False)
    rows = grouped2.to_dict('records')
    return rows
